...
it('converts xy color to rgb color', () => {
  ...
  expect(
    HueColor.fromXyToRgb(
      [0.32272672086556803, 0.3290229095590793],
      53.90655847655765
    )
  ).toEqual([127, 127, 127]);
  expect(
    HueColor.fromXyToRgb([0.7006062331309042, 0.2993009868421053], 72.105774)
  ).toEqual([255, 0, 0]);
  expect(
    HueColor.fromXyToRgb(
      [0.1724161431490603, 0.7467966085220437],
      169.78198200000003
    )
  ).toEqual([0, 255, 0]);
  expect(
    HueColor.fromXyToRgb(
      [0.13550301400290363, 0.03987867049354716],
      12.111989999999999
    )
  ).toEqual([0, 0, 255]);
});
...
it('converts rgb color to xy color', () => {
  expect(HueColor.fromRgbToXy([255, 255, 255])).toEqual([
    0.32272672086556803,
    0.32902290955907926,
    253.99974600000002
  ]);
  expect(HueColor.fromRgbToXy([0, 0, 0])).toEqual([0, 0, 0]);
  expect(HueColor.fromRgbToXy([127, 127, 127])).toEqual([
    0.32272672086556803,
    0.3290229095590793,
    53.90655847655765
  ]);
  expect(HueColor.fromRgbToXy([255, 0, 0])).toEqual([
    0.7006062331309042,
    0.2993009868421053,
    72.105774
  ]);
  expect(HueColor.fromRgbToXy([0, 255, 0])).toEqual([
    0.1724161431490603,
    0.7467966085220437,
    169.78198200000003
  ]);
  expect(HueColor.fromRgbToXy([0, 0, 255])).toEqual([
    0.13550301400290363,
    0.03987867049354716,
    12.111989999999999
  ]);
